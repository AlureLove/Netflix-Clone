...
class MainTabBarViewController: UITabBarController {
    ...
    override var supportedInterfaceOrientations: UIInterfaceOrientationMask {
        // If we have a presented view controller (like TitlePreviewViewController), defer to it
        if let presented = presentedViewController {
            return presented.supportedInterfaceOrientations
        }

        // Check if current navigation stack has TitlePreviewViewController
        if let nav = selectedViewController as? UINavigationController,
           let topVC = nav.topViewController {
            if topVC is TitlePreviewViewController {
                return .allButUpsideDown
            }
        }

        // Default: only portrait for browsing
        return .portrait
    }
