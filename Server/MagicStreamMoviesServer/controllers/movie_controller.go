...
import (
	...
	"github.com/AlureLove/Netflix-Clone/Server/MagicStreamMoviesServer/database"
	"github.com/AlureLove/Netflix-Clone/Server/MagicStreamMoviesServer/models"
	"github.com/AlureLove/Netflix-Clone/Server/MagicStreamMoviesServer/utils"
...
func AdminReviewUpdate() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		if role != "ADMIN" {
			c.JSON(http.StatusUnauthorized, gin.H{"error": "User must be part of ADMIN role to update"})
			return
		}
...
func GetReviewRanking(admin_review string) (string, int, error) {
	...
	if openAiApiKey == "" {
		return "", 0, errors.New("OpenAI API key not found")
	}
	...
	base_prompt_template := os.Getenv("BASE_PROPERTIES")
